...
function gestionarErrores(response) {
    if (!response.ok) {
        throw Error("ERROR EN LA PETICIÓN:" + response.statusText);
    }
    return response;
}
...
function realizarPeticionAsincrona3() {
    let provincias = document.getElementById("distribuciones").value;
    ...
    let datos = "distribuciones=" + distribuciones;
